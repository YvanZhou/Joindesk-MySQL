...
//        entityManager.createNativeQuery("update issue set content_vector = (to_tsvector('" + issue.getKeyPair() + "') || to_tsvector(summary)) where id=" + issue.getId()).executeUpdate();
        entityManager.createNativeQuery("update issue set content_vector = CONCAT('" + issue.getKeyPair() + "',summary) where id=" + issue.getId()).executeUpdate();
    }
}

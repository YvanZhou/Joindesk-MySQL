...
import com.ariseontech.joindesk.bo.IssueSearchData;
        ...
        if (issueSearchDTO.getFilter().getQuery().getRules() != null) {
            issueSearchDTO.getFilter().getQuery().getRules().forEach(rule -> {
                ...
                if (rule.getField().equalsIgnoreCase("created") ||
                    ...
                    switch (rule.getOperator()) {
                        case "IN":
                            issueSearchData.setOperatorType("IN");
                            if (rule.getField().equalsIgnoreCase("current_step")) {
                                ...
                                for (String value : rule.getValues()) {
                                    ...
                                    issueSearchData.addData(rule.getField(), Long.parseLong(value));
                                }
//                                q.append(" (").append(sq.toString()).append(") ");
                            } else
                            ...
                            break;
                        case "NOT IN":
                            issueSearchData.setOperatorType("NOT IN");
                            if (rule.getField().equalsIgnoreCase("current_step")) {
                                List<String> values = new ArrayList<>();
                                workflowService.getAllWorkflowSteps().stream().filter(ws -> Arrays.asList(rule.getValues()).contains(ws.getIssueStatus().getId().toString())).forEach(workflowStep -> values.add(workflowStep.getId().toString()));
...
//        String textFilter = !ObjectUtils.isEmpty(issueSearchDTO.getFilter().getSearchQuery()) ? " AND content_vector @@ to_tsquery('" + issueSearchDTO.getFilter().getSearchQuery().replaceAll(" ", "|") + "')" : "";
        String textFilter = !ObjectUtils.isEmpty(issueSearchDTO.getFilter().getSearchQuery()) ? " AND content_vector LIKE'%" + issueSearchDTO.getFilter().getSearchQuery() + "%'" : "";
        ...
        issueSearchDTO.setIssues(issueSearchData.filterIssues(issueSearchCustomRepo.filter(finalQuery, backingQuery)));
        issueSearchDTO.setTotal(issueSearchData.filterIssues(issueSearchCustomRepo.filter(finalQuery, "")).size());
...
//                i.setKey(getLastKeyForProject(project) + 1);
                Long max = issueRepo.findLastKeyForProject(project);
                i.setKey(max != null ? (max + 1) : 1);
...
//                '"' + (customField.getCustomField().isMultiple() ? String.join(",", customField.getValues()) : customField.getValue()) + '"');
        issueCustomRepo.updateCustomDataByIssue(issue.getId(), customField.getCustomField().getKey(), (customField.getCustomField().isMultiple() ? String.join(",", customField.getValues()) : customField.getValue()));
    ...
    public IssueUpdate update(Issue is, Long issueKey, String projectKey) {
        ...
        Optional.ofNullable(is.getStartDate()).ifPresent(i -> {
            ...
            if (parentIssue != null && (parentIssue.getStartDate() == null || issue.getStartDate().isBefore(parentIssue.getStartDate()))) {
                parentIssue.setStartDate(issue.getStartDate());
                save(parentIssue);
            }
        });
        Optional.ofNullable(is.getEndDate()).ifPresent(i -> {
            ...
            // 父任务结束时间等于子任务最晚结束时间
            Issue parentIssue = issue.getParentIssue();
            if (parentIssue != null && (parentIssue.getEndDate() == null || issue.getEndDate().isAfter(parentIssue.getEndDate()))) {
                parentIssue.setEndDate(issue.getEndDate());
                save(parentIssue);
            }
        });
...
//            throw new JDException("", ErrorCode.FORBIDDEN, HttpStatus.FORBIDDEN);
    ...
    public IssueUpdate createSubIssue(Issue issue, Long projectID, Long parentIssueKey) {
        Project project = projectService.getProject(projectID);
        if (!canCreate(project)) {
            throw new JDException("cant create issue in this project", ErrorCode.FORBIDDEN, HttpStatus.FORBIDDEN);
        }
        issue.setProject(project);

        Issue parentIssue = get(project.getKey(), parentIssueKey);
        if (parentIssue == null) {
            throw new JDException("parent issue not found", ErrorCode.ISSUE_NOT_FOUND, HttpStatus.NOT_FOUND);
        }

        Optional<IssueType> issueType = issueTypeRepo.findById(issue.getIssueType().getId());
        if (issueType.isEmpty()) {
            throw new JDException("issue type not found", ErrorCode.ISSUE_TYPE_NOT_FOUND, HttpStatus.NOT_FOUND);
        }
...
//                i.setKey(getLastKeyForProject(project) + 1);
                ...
                i.setKey(max != null ? (max + 1) : 1);
                i.setKeyPair(project.getKey() + "-" + i.getKey());
                i.setParentIssue(parentIssue);
                i = save(i);

                List<Issue> subIssues = parentIssue.getSubIssues();
                if (subIssues == null) {
